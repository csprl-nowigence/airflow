...
# scrape_web(base_dir="/Users/nowigence/PycharmProjects/airflow_exploration/output", execution_date=dt.datetime.now(tz=dt.timezone.utc))
def scrape_web(**kwargs):
    ...
    weeks = {}
    r = requests.get('http://idsp.nic.in/index4.php?lang=1&level=0&linkid=406&lid=3689', verify=False)
    ...
    if os.path.exists(os.path.join(base_dir, 'week.csv')):
        ...
        weeks_to_download = sorted(weeks.keys(), key=lambda x: int(x))
...
def scrape_pdf(**kwargs):
    ...
    if len(pdfs) > 0:
        for pdf in pdfs:
            print(f"splitting {os.path.basename(pdf)}")
